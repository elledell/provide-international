...
export default function DonatePage() {
  ...
  const currencySymbol = paymentMethod === "mpesa" ? "KES " : "$";

  const createPendingDonation = useMutation(api.donations.createPendingDonation);

  // Reactively watches the donation record — flips to success/failed the instant
  // the /api/mpesa/callback route patches it in Convex. No polling.
  ...
  const handleDonate = async () => {
    ...
    if (paymentMethod !== "mpesa") {
      // Card/PayPal aren't wired up in this demo — production will route both
      // through Pesapal. M-Pesa is the flow being demonstrated here.
      setErrorMessage(
        `${paymentMethod === "card" ? "Card" : "PayPal"} checkout isn't connected in this demo — select M-Pesa to see the live payment flow.`
      );
      return;
    }

    if (!phone.match(/^254\d{9}$/)) {
      setErrorMessage("Enter the phone number as 2547XXXXXXXX (no + or leading 0).");
      return;
    }
    ...
    try {
      ...
      if (!result) {
        setErrorMessage("Could not send the STK push — check the server logs.");
        return;
      }

      await createPendingDonation({
        donorName: `${firstName || "Generous"} ${lastName || "Donor"}`.trim(),
      ...
      setErrorMessage("Something went wrong initiating the payment.");
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="w-full lg:w-7/12 order-1 lg:order-2">
              <div className="bg-white rounded-[2rem] shadow-2xl p-6 sm:p-10 border border-slate-100">
                ...
                <div className="mb-8">
                  ...
                  {amount === 0 && (
                    <div className="relative mt-4 animate-fade-in">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl py-4 pl-14 pr-4 font-bold text-lg text-[#152033] focus:outline-none focus:border-[#FFC107]"
                        autoFocus
                      />
                    </div>
                  )}
                  ...
                  <p className="text-sm text-slate-500 mt-4 text-center bg-blue-50/50 py-3 rounded-lg border border-blue-100">
                    {donationType === "monthly"
                      ? `A monthly gift of ${currencySymbol}${amount || "..."} helps sustain long-term community programs.`
                      : `Your one-time gift of ${currencySymbol}${amount || "..."} provides immediate relief to those in need.`}
                  </p>
                </div>
                ...
                <div className="mb-8 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#395684]"
                    />
                    <input
                      type="text"
                  ...
                  {paymentMethod === "mpesa" && (
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-green-600"
                    />
                  )}
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full bg-[#FFC107] hover:bg-yellow-400 text-[#152033] font-black text-xl tracking-wide uppercase py-5 rounded-xl shadow-[0_10px_20px_rgba(255,193,7,0.3)] hover:shadow-[0_15px_25px_rgba(255,193,7,0.4)] hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-3 disabled:opacity-60 disabled:translate-y-0"
                >
                  ...
                  {loading ? "SENDING PROMPT..." : `DONATE ${amount > 0 ? `${currencySymbol}${amount}` : ""}`}
                </button>

                {/* Live status — updates automatically when the M-Pesa callback lands in Convex */}
                {checkoutRequestId && (
                  <div className="mt-4 text-sm p-4 rounded-lg bg-slate-50 border border-slate-200 text-center">
                    {!donation || donation.status === "pending" ? (
                      <p className="text-[#395684] font-semibold">Check your phone and enter your M-Pesa PIN…</p>
